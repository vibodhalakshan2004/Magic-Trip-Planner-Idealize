...
import { Home, Hotel as HotelIcon, RefreshCw } from "lucide-react";
    ...
    <div className="grid gap-5">
      ...
      <div className="grid gap-4 rounded-md border border-slate-200 bg-slate-50 p-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button disabled={busy} onClick={() => run(async () => {
            const response = await hotelApi.suggestDailyHotels(tripId, activeDay, { hotel_type: hotelType, hotel_preference: manualQuery || preference, rooms, max_results: 8 });
            ...
            <div className="grid gap-0 sm:grid-cols-[150px_1fr]">
              <div className="h-40 bg-slate-100 sm:h-full">
                {hotel.image_url ? (
                  ...
                  <img src={`/api/image?url=${encodeURIComponent(hotel.image_url)}`} alt={hotel.name} className="h-full w-full object-cover" />
                ) : (
                  <div className="grid h-full place-items-center text-slate-400"><HotelIcon className="h-8 w-8" /></div>
                )}
              </div>
              <div className="p-4">
      ...
      {!busy && suggestions.length === 0 ? (
        <div className="rounded-md border border-dashed border-slate-300 bg-white p-5 text-sm font-semibold text-slate-500">
          Search by hotel name, area, city, or location to find accommodation for this day.
        </div>
      ) : null}
